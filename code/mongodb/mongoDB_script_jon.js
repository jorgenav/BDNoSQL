...
db.practica.aggregate([{$match:{"Year":2014}},
                   {$group:{_id:"$Year","docsTotal":{$sum:1},"docsType":{$push:"$doc_type"}}}])
                   
db.practica.aggregate([{$match:{"Year":2014,"Type":"article"}},
...
db.practica.aggregate([{$unwind:"$Authors"},
                   {$match:{"Type":"incollection","Authors.Nombre":{$ne:null}}},
                   {$sortByCount:"$Authors.Nombre"},
                   {$limit:10}])
...
db.practica.aggregate([{$match:{"Authors.Nombre":"Luca Cabibbo"}},
                   {$bucket:
                       {groupBy: "$Auhtors.Nombre",
                        boundaries: [0,1],
                        default: "Luca Cabibbo",
                        output:{"coauthor": {$push: "$Auhtors.Nombre"}}}},
                    {$unwind:"$coauthor"},
...
db.practica.find({Year: {$exists: true}}).forEach(function(obj) { 
    obj.year = new NumberInt(obj.year);
    db.docs.save(obj);
});


db..aggregate([{$unwind:"$author"},
